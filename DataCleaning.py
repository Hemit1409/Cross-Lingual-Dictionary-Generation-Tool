...
def Tokenization():
    ...
    with open('IntermediateOutput.txt', 'w', encoding='utf-8') as IHO:
...
def stopWordsRemoval():
    global Output
    with open('IntermediateOutput.txt', 'r', encoding='utf-8') as IO:
        InputDataFrom_IO = IO.readlines()

    if LanguageCode == 'gu':
        with open('GujaratiStopWords.txt', 'r', encoding='utf-8') as SW:
            InputDataFrom_SW = SW.readlines()
        Output = open('GujaratiOutput.txt', 'w', encoding='utf-8')
    elif LanguageCode == 'hi':
        with open('HindiStopWords.txt', 'r', encoding='utf-8') as SW:
            InputDataFrom_SW = SW.readlines()
        Output = open('HindiOutput.txt', 'w', encoding='utf-8')
